...
my %current_data = (
    epoch              => $obs->{epoch},
    temperature        => sprintf("%.1f", $obs_m->{temp}),
    feelslike          => sprintf("%.1f", $obs_m->{windChill}),
    ...
    windchill          => sprintf("%.1f", $obs_m->{windChill}),
    pressure           => $obs_m->{pressure},
    dewpoint           => $obs_m->{dewpt},
    solar_radiation    => sprintf("%.0f", $obs->{solarRadiation} // 0),
    uv_index           => $obs->{uv},
